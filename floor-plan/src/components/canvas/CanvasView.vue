...
type DragState =
  ...
  | { kind: 'dim-handle'; id: string; handle: 'start' | 'end' | 'offset'; startWorld: Pt }
  | { kind: 'fur-rotate'; id: string; cx: number; cy: number }
...
/** 收集所有墙端点用于吸附 */
function snapCandidates(): { p: Pt; ownerId: string }[] {
  const out: { p: Pt; ownerId: string }[] = []
  for (const w of walls.value) {
    w.points.forEach((p) => out.push({ p, ownerId: w.id }))
  }
  return out
}

function applySnap(p: Pt, excludeWallId?: string): { point: Pt; snapped: boolean } {
  if (!state.doc.settings.snapEnabled) return { point: p, snapped: false }
  const cands = snapCandidates().filter((c) => c.ownerId !== excludeWallId)
  const r = snapToPoints(
    p,
    cands.map((c) => c.p),
    snapTolMm()
  )
  if (r) {
    return { point: { ...r.point }, snapped: true }
  }
...
function onPointerDown(e: PointerEvent) {
  ...
  switch (state.mode) {
    ...
    case 'window':
      ...
      break
    case 'dimension':
      handleDimensionClick(e, w)
      break
    case 'measure':
      handleMeasureClick(e, w)
      break
  }
}
...
function handleSelectDown(e: PointerEvent, w: Pt) {
  ...
  if (!hit) {
    ...
    drag.current = { kind: 'marquee', x0: w.x, y0: w.y }
  ...
  if (el.kind === 'dimension') {
    if (hit.kind === 'dim-handle' && hit.handle) {
        drag.current = { kind: 'dim-handle', id: el.id, handle: hit.handle, startWorld: w }
      return
    }
  ...
  if (el.kind === 'furniture') {
    ...
    if (rotHandle && Vec2.dist(rotHandle, w) <= snapTolMm() * 1.2) {
        drag.current = { kind: 'fur-rotate', id: el.id, cx: el.x, cy: el.y }
      return
    }
...
function handleWallClick(e: PointerEvent, wRaw: Pt) {
  ...
  if (draft.length > 0) {
    ...
    w = orthoPoint(prev, w, state.doc.settings.ortho)
    const s = applySnap(w)
    w = s.point
    rawState.snapMarker = s.snapped ? { ...w } : null
...
function updateWallHover(wRaw: Pt) {
  const draft = rawState.wallDraft
  if (draft.length === 0) {
    const s = applySnap(wRaw)
    rawState.snapMarker = s.snapped ? { ...s.point } : null
    wallCursor.value = s.point
    return
  }
  const prev = draft[draft.length - 1]
  let w = orthoPoint(prev, wRaw, state.doc.settings.ortho)
  const s = applySnap(w)
  w = s.point
  rawState.snapMarker = s.snapped ? { ...w } : null
  wallCursor.value = w
...
function handleDimensionClick(e: PointerEvent, w: Pt) {
  const s = applySnap(w)
  w = s.point
  if (!dimFirst.value) {
    dimFirst.value = { ...w }
...
function handleMeasureClick(e: PointerEvent, w: Pt) {
...
function doWallVertex(w: Pt, d: Extract<DragState, { kind: 'wall-vertex' }>) {
  beginDragMutation()
  const wall = wallMap.value.get(d.wallId)
  if (!wall) return
  const s = applySnap(w, wall.id)
  let p = s.point
  p = orthoVertex(wall, d.index, p)
  wall.points[d.index] = { ...p }
}

/** 拖动墙端点时，按 Shift/全局正交开关将相邻段锁定为水平/垂直 */
function orthoVertex(wall: WallElement, index: number, p: Pt): Pt {
  if (!state.doc.settings.ortho) return p
  // 与前一个顶点对齐
  const prev = wall.points[index - 1]
  const next = wall.points[index + 1]
  if (prev && next) {
    // 中间点：保持两段总长思路，只吸附到水平/垂直方向（以更长段为准）
    return p
  }
  const ref = prev ?? next
  if (!ref) return p
...
function doDimHandle(w: Pt, d: Extract<DragState, { kind: 'dim-handle' }>) {
  ...
  const s = applySnap(w)
  w = s.point
  if (d.handle === 'start') el.p1 = { ...w }
  else if (d.handle === 'end') el.p2 = { ...w }
  else {
...
function doFurRotate(w: Pt, d: Extract<DragState, { kind: 'fur-rotate' }>) {
  beginDragMutation()
  ...
  let snapped = Math.round(ang / step) * step
  el.rotation = snapped
}
