...
export function orthoPoint(prev: Pt, raw: Pt, enabled: boolean, thresholdDeg = 18): Pt {
  ...
  if (Math.hypot(dx, dy) < EPS) return raw
  const ang = Math.atan2(dy, dx)
  const step = Math.PI / 2
  const nearest = Math.round(ang / step) * step
  let diff = Math.abs(((ang - nearest + Math.PI * 3) % (Math.PI * 2)) - Math.PI)
  if (diff < 0) diff = -diff
  if (Math.abs(diff) <= (thresholdDeg * Math.PI) / 180) {
    const l = Math.hypot(dx, dy)
    return { x: prev.x + Math.cos(nearest) * l, y: prev.y + Math.sin(nearest) * l }
  }
  return raw
}
